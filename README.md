...
В директории src
В файле database.py напиши асинхронный пул подключения к БД в возможностью указания количeства коннектов к базе данных.

Реализуй async def get_db_connection() для использования коннекта в dependencies.py.

Подготовь в диреткории tests файл conftest.py для асинхронных тестов. При старте тестов, должно происходить подключение к тестовой базе данных, удаление существующих в ней таблиц, установка актуальных миграций через alembic, генерация новых тестовых данных.